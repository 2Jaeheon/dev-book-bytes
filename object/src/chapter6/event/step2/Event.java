...
public class Event {
    ...
    // 반환 값을 돌려주는 메서드는 쿼리이므로 부수효과에 대한 부담이 없다.
    // 하지만 반환 값을 가지지 않는 메서드는 모두 명령이므로 부수효과에 주의해야 한다.
    // 반환 값을 가지는지 여부만 확인하면 간단함.
    // 이렇게 하면 Event 객체를 사용하는 쪽에서 reschedule을 결정할 수 있다.
